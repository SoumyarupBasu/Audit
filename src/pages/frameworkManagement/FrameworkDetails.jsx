...
function RequirementCard({ requirement, complianceStatus }) {
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case "partial":
        return <span className="icon icon-warning"></span>;
      case "not-met":
...
function ComparisonResults({ framework, documentFile, complianceData }) {
  const totalRequirements = framework.sections.reduce(
    (total, section) => total + section.requirements.length,
    0
  );
  const metRequirements = complianceData.filter(
    (item) => item.status === "met"
  ).length;
  const partialRequirements = complianceData.filter(
    (item) => item.status === "partial"
  ).length;
  const notMetRequirements = complianceData.filter(
    (item) => item.status === "not-met"
  ).length;
          ...
          <div className="compare-left-head">
            ...
            <div className="doc-chip">
              ...
              <span className="doc-name">
                {documentFile?.name || "Document.pdf"}
              </span>
            </div>
          </div>
              ...
              <div key={section.id} className="section-breakdown">
                  ...
                  <span className="section-compliance">
                    {
                      complianceData.filter(
                        (item) =>
                          item.sectionId === section.id && item.status === "met"
                      ).length
                    }
                ...
                <div className="requirements-list">
                  {section.requirements.map((requirement) => {
                    const complianceItem = complianceData.find(
                        ...
                        <div className="compare-cell document-side">
                          <div
                            className={`status-pill ${
                              complianceItem?.status || "unknown"
                            }`}
                          >
                            {complianceItem?.status === "met" && "✓ Met"}
                            {complianceItem?.status === "partial" &&
                              "⚠ Partial"}
                            {complianceItem?.status === "not-met" &&
                              "✗ Not Met"}
                            {!complianceItem && "○ Unknown"}
                          </div>
                          <div className="doc-snippet">
                            Relevant excerpt preview will appear here.
                          </div>
                        </div>
                      </div>
                    );
        ...
        <aside className="compare-right">
          ...
          <div className="legend-card">
            ...
            <div className="legend-items">
              ...
              <div className="legend-item">
                <span className="dot not-met"></span> Not Met
              </div>
              <div className="legend-item">
              ...
              <div className="doc-meta">
                ...
                <div className="doc-sub">
                  {documentFile?.size
                    ...
                    : "—"}{" "}
                  MB • {documentFile?.type || "application/pdf"}
                </div>
              </div>
            </div>
          </div>
        ...
        <main className="content">
          <div className="container">
            <div className="error-state">
              ...
              <button className="primary" onClick={handleBackToSelection}>
                Go Back
              </button>
            </div>
          </div>
        ...
        <div className="container stack gap-lg">
          ...
          <Section title="Framework Information">
            <div className="framework-header">
              ...
              >
                <span
                  className={`icon ${selectedFramework.icon}`}
                  style={{ fontSize: "48px", color: "white" }}
                ></span>
              </div>
              <div className="framework-info">
                <h2 className="framework-name">{selectedFramework.name}</h2>
                <h3 className="framework-full-name">
              ...
              {selectedFramework.isCustom && similarFrameworks.length > 0 && (
                <Section title="Similar Predefined Frameworks">
                  <div className="hint" style={{ marginBottom: "16px" }}>
                    Based on your custom framework, we found{" "}
                    {similarFrameworks.length} similar predefined framework
                    {similarFrameworks.length > 1 ? "s" : ""} that may provide
                    additional compliance insights.
                  </div>

                  {similarFrameworksComplianceData.map(
                    ({ framework, similarity, complianceData }) => {
                      const totalReqs = framework.sections.reduce(
                        (sum, s) => sum + s.requirements.length,
                        0
                      );
                      const metReqs = complianceData.filter(
                        (item) => item.status === "met"
                      ).length;
                      const compliancePercentage = Math.round(
                        (metReqs / totalReqs) * 100
                      );
                      const reasons = getSimilarityReasons(
                        selectedFramework,
                        framework
                      );
