...
export default function FrameworkComparison({
  ...
  const getMatchingFeatures = (framework) => {
    const customFeatures = new Set(
      (uploadedFramework.features || []).map((f) => f.toLowerCase())
    );
        ...
        <div className="container stack gap-lg">
          ...
          <Section title="Your Custom Framework">
            <div className="framework-header-card">
              <div
                className="framework-icon-large"
                style={{ backgroundColor: uploadedFramework.color }}
              >
                <Icon
                  name={uploadedFramework.icon}
                  size="48px"
                  style={{ color: "white" }}
                />
              </div>
              <div className="framework-header-info">
                <h2 className="framework-name-large">
                  {uploadedFramework.name}
                </h2>
                <p className="framework-full-name">
                  {uploadedFramework.fullName || uploadedFramework.name}
                </p>
                <div className="framework-meta">
                  <span className="meta-badge">
                    {uploadedFramework.category}
                  </span>
                  <span className="meta-item">
                    {uploadedFramework.sections?.length || 0} Sections
                  </span>
                  <span className="meta-item">
                    {uploadedFramework.sections?.reduce(
          ...
          <Section
            ...
            right={
              <span className="hint-text">
                Showing all {frameworkComparisons.length} preset frameworks
                ranked by similarity
              </span>
            }
          >
            <div className="similarity-grid">
              {frameworkComparisons.map(
                ({ framework, similarity, reasons }, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="similarity-card-header">
                        <div
                          className="framework-icon-small"
                        ...
                        <div className="similarity-card-info">
                          <h4 className="similarity-framework-name">
                            {framework.name}
                          </h4>
                          <p className="similarity-framework-category">
                            {framework.category}
                          </p>
                        </div>
                        <div
                          className="similarity-score-badge"
                          style={{
              ...
              <table className="comparison-table">
                ...
                <tbody>
                      ...
                      <td key={framework.id} className="comparison-value-cell">
                        <span
                          className={`value-badge ${
                            framework.category === uploadedFramework.category
                              ? "match"
                              : ""
                          }`}
                        >
                  ...
                  <tr>
                    ...
                    <td className="comparison-value-cell custom-column">
                      {uploadedFramework.sections?.length || 0}
                    </td>
                    {topSimilarFrameworks.map(({ framework }) => (
                      <td key={framework.id} className="comparison-value-cell">
                        {framework.sections?.length || 0}
          ...
          <Section title="Upload Document for Audit">
            <div className="upload-hint">
              ...
              <span>
                Upload a document to analyze compliance against your custom
                framework and the top similar preset frameworks
              </span>
            </div>

            <div
              className={`dropzone-modern ${documentFile ? "has-file" : ""}`}
              onDrop={onDrop}
