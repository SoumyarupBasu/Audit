...
export default function ComparisonResults({ jobId, framework, onNavigate, onBack }) {
  ...
  async function loadResults() {
    ...
    setError(null)
    try {
      const response = await fetch(`http://localhost:3001/api/compare/results/${jobId}`)
      if (!response.ok) {
  ...
  function getFilteredComparisons() {
    ...
    if (filter === 'high') {
      ...
      filtered = filtered.filter(c => c.similarityPercent >= 70 && c.similarityPercent < 90)
    } else if (filter === 'low') {
    ...
    if (sortBy === 'score') {
      ...
      filtered.sort((a, b) => (a.clientControl.name || '').localeCompare(b.clientControl.name || ''))
    }
  ...
  function exportToCSV() {
    ...
    const headers = ['Client Control', 'Client Description', 'Framework Control ID', 'Framework Control', 'Framework Description', 'Similarity Score']
    ...
    a.href = url
    a.download = `comparison-${results.frameworkId}-${new Date().toISOString().split('T')[0]}.csv`
    a.click()
    ...
    <div className="comparison-results-page">
      ...
      <div className="summary-cards">
        <div className="summary-card">
          <div className="card-icon doc"><Icon name="file-text" size="24px" /></div>
          <div className="card-content">
            <div className="card-value">{results?.totalDocumentControls || 0}</div>
            <div className="card-label">Document Controls</div>
          </div>
        </div>
        <div className="summary-card">
          <div className="card-icon fw"><Icon name="shield" size="24px" /></div>
          <div className="card-content">
            <div className="card-value">{results?.totalFrameworkControls || 0}</div>
            <div className="card-label">Framework Controls</div>
          </div>
        </div>
        <div className="summary-card high">
          <div className="card-icon"><Icon name="check-circle" size="24px" /></div>
          <div className="card-content">
        ...
        <div className="summary-card medium">
          <div className="card-icon"><Icon name="alert-circle" size="24px" /></div>
          <div className="card-content">
            <div className="card-value">{results?.summary?.mediumMatch || 0}</div>
            <div className="card-label">Medium Match (70-89%)</div>
          </div>
        </div>
        <div className="summary-card low">
          <div className="card-icon"><Icon name="x-circle" size="24px" /></div>
          <div className="card-content">
        ...
        <div className="summary-card avg">
          <div className="card-icon"><Icon name="bar-chart-2" size="24px" /></div>
          <div className="card-content">
            <div className="card-value">{results?.summary?.averageScore || 0}%</div>
        ...
        <table className="comparison-table">
          ...
          <tbody>
            {filteredComparisons.map((comp, idx) => (
              <tr key={idx} className={`match-${comp.similarityPercent >= 90 ? 'high' : comp.similarityPercent >= 70 ? 'medium' : 'low'}`}>
                <td className="col-score">
                  <ScoreBadge score={comp.similarityPercent} />
                </td>
                <td className="col-client">
                  <div className="control-card client">
                    <div className="control-name">{comp.clientControl.name || comp.clientControl.id || 'Unnamed Control'}</div>
                    <div className="control-desc">{comp.clientControl.description?.substring(0, 200)}{comp.clientControl.description?.length > 200 ? '...' : ''}</div>
                  </div>
                </td>
                ...
                <td className="col-framework">
                  <div className="control-card framework">
                    ...
                    <div className="control-name">{comp.frameworkControl.title}</div>
                    <div className="control-desc">{comp.frameworkControl.description?.substring(0, 200)}{comp.frameworkControl.description?.length > 200 ? '...' : ''}</div>
                  </div>
                </td>
              </tr>
            ))}
