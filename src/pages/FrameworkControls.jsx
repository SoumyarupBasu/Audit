...
function Breadcrumbs({ onNavigate, frameworkName }) {
  return (
    <div className="crumbs">
      ...
      <span className="crumb-link" onClick={() => onNavigate('framework')}>Browse Frameworks</span>
      <span className="sep">/</span>
...
export default function FrameworkControls({ framework, onBack, onNavigate }) {
  ...
  async function loadControls() {
    ...
    try {
      ...
      if (framework.id?.startsWith('custom-') || framework.isAIExtracted) {
        ...
        const response = await fetch(`http://localhost:3001/api/custom-frameworks/${framework.id}/controls`)
        if (response.ok) {
        ...
        const supportedFrameworks = ['iso27001', 'nist', 'sox', 'cis', 'gdpr', 'pci-dss', 'ai-nlp']
        ...
        if (supportedFrameworks.includes(framework.id)) {
          ...
          const response = await fetch(`http://localhost:3001/api/frameworks/${framework.id}/controls`)
          if (response.ok) {
            const data = await response.json()
            setControls(data.controls || [])
          } else {
  ...
  useEffect(() => {
    if (filteredControls.length > 0 && localSelectedControls.size === filteredControls.length) {
      setSelectAll(true)
    } else {
      setSelectAll(false)
    }
  ...
  const handleBuildCustomFramework = () => {
    const selectedControlsArray = controls.filter(c => localSelectedControls.has(c.id))
  ...
  const handleEditControl = async (updatedControl) => {
    try {
      const response = await fetch(`http://localhost:3001/api/frameworks/${framework.id}/controls/${editingControl.id}`, {
        method: 'PUT',
      ...
      // Update local state
      setControls(prev => prev.map(c => c.id === editingControl.id ? updatedControl : c))
  ...
  const handleDeleteControl = async () => {
    try {
      const response = await fetch(`http://localhost:3001/api/frameworks/${framework.id}/controls/${deletingControl.id}`, {
        method: 'DELETE'
      })
      ...
      <main className="content">
        <header className="page-head">
          <div className="container head-grid">
            <div className="page-title">
              <Icon name="shield" size="28px" style={{ marginRight: '12px', color: framework.color }} />
              {framework.name} Controls
            </div>
            <div className="page-actions">
              <button className="btn-add-control" onClick={() => setShowAddModal(true)}>
            ...
            <Breadcrumbs onNavigate={onNavigate} frameworkName={framework.name} />
          </div>
        </header>
        ...
        <div className="container controls-container">
          ...
          <div className="framework-info-card">
            <div className="framework-info-header">
              <div className="framework-info-icon" style={{ backgroundColor: framework.color }}>
              ...
              <div className="framework-info-details">
                ...
                <p className="framework-info-description">{framework.description}</p>
              </div>
            </div>
            ...
            <div className="select-all-section">
              ...
              {localSelectedControls.size > 0 && (
                <div className="selection-counter">
                  {localSelectedControls.size} control{localSelectedControls.size !== 1 ? 's' : ''} selected
                </div>
              )}
            </div>
          )}
          ...
          {!loading && controls.length === 0 && (
            <div className="no-controls">
              ...
              <p>Controls for this framework are not yet available in the system.</p>
            </div>
          )}
          ...
          {!loading && controls.length > 0 && filteredControls.length === 0 && (
            <div className="no-results">
              ...
              <p>No controls match your current filters. Try adjusting your search criteria.</p>
              ...
              {Object.entries(groupedControls).map(([category, categoryControls]) => (
                <div key={category} className="controls-category">
                  <div className="category-header">
                    ...
                    <span className="category-count">{categoryControls.length} controls</span>
                  </div>
                  <div className="category-controls">
                    {categoryControls.map(control => (
                      <div
                        key={control.id}
                        className={`control-card ${localSelectedControls.has(control.id) ? 'selected' : ''}`}
                      >
                        ...
                        <div className="control-header">
                          <div className="control-id-badge" style={{ borderColor: framework.color }}>
                            {control.id}
                          </div>
        ...
        {localSelectedControls.size > 0 && (
          <div className="build-framework-sticky">
            <button className="btn-build-framework" onClick={handleBuildCustomFramework}>
              <Icon name="plus" size="20px" />
              {isAddingFromFramework ? 'Add to Custom Framework' : 'Build Custom Framework'}
              <span className="build-count-badge">{localSelectedControls.size}</span>
            </button>
          </div>
        )}
