...
export default function UploadDocument() {
  const navigate = useNavigate();
  // Local state - only used by this component
  ...
  function handleUpload() {
    ...
    // Navigate to framework selection with the uploaded file
    navigate("/framework", { state: { selectedFile: file } });
  }
  ...
  async function handleFrameworkSelect(framework) {
    ...
    try {
      ...
      const uploadResponse = await fetch(
        "http://localhost:3001/api/compare/upload",
        {
          method: "POST",
          body: formData,
        }
      ...
      while (!completed) {
        ...
        const statusResponse = await fetch(
          `http://localhost:3001/api/compare/status/${jobId}`
        );
        ...
        if (status.status === "completed") {
          ...
          navigate("/comparison-results", { state: { jobId, framework } });
      ...
      <main className="content">
        ...
        <div className="container stack gap-lg">
          <Section title="Document Files">
            <div className="hint">
              Upload one document file. Supported: PDF, DOC, DOCX, PNG, JPEG,
              GIF, MP3, MP4, AAC, OGG, AMR, 3GPP, MOV. Max: 50MB.
            </div>
            ...
            >
              ...
              <div className="drop-sub">
                ...
                image/png, image/jpeg, image/gif, audio/mpeg, audio/mp4,
                audio/aac, audio/ogg, audio/amr, video/mp4, video/3gpp,
                video/quicktime - Max size: 50MB
              </div>
              <input
                type="file"
                ref={inputFileRef}
            ...
            <div className="legend">
              <div>
                ...
                <span className="muted">
                  PDF, DOC, DOCX, PNG, JPEG, GIF, MP3, MP4, AAC, OGG, AMR, 3GPP,
                  MOV
                </span>
              </div>
              <div>
                Maximum file size: <span className="muted">50MB</span>
              </div>
              <div>
                Upload limit: <span className="muted">One file only</span>
              </div>
            </div>
          </Section>
          ...
          <Section title="Document Information">
            ...
            <div className="grid2">
              <div className="field">
                ...
                <input
                  ...
                  min="0"
                  max="10"
                  value={form.sourceSeverity}
                  onChange={(e) =>
                    update("sourceSeverity", parseFloat(e.target.value))
                  }
                  className="range"
                />
                <div className="muted">
                  Current value: {form.sourceSeverity}
                </div>
              </div>
              <div className="field">
                ...
                <input
                  ...
                  min="0"
                  max="10"
                  value={form.reviewSeverity}
                  onChange={(e) =>
                    update("reviewSeverity", parseFloat(e.target.value))
                  }
                  className="range"
                />
                <div className="muted">
                  Current value: {form.reviewSeverity}
                </div>
              </div>
            </div>
              ...
              <div className="comparison-progress">
                ...
                <div className="progress-info">
                  <span className="progress-phase">
                    {comparisonProgress.phase === "extracting" &&
                      "Extracting controls from document..."}
                    {comparisonProgress.phase === "comparing" &&
                      "Comparing with framework controls..."}
                    {comparisonProgress.phase === "uploading" &&
                      "Uploading document..."}
                    {comparisonProgress.phase === "complete" &&
                      "Comparison complete!"}
                  </span>
                  <span className="progress-percent">
                    {comparisonProgress.percent}%
                  </span>
                </div>
              </div>
            )}
            ...
            {comparisonError && (
              <div className="comparison-error">
                ...
                <button onClick={() => setComparisonError(null)}>
                  Dismiss
                </button>
              </div>
            )}
