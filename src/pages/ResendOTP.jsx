...
export default function ResendOTP({ onNavigate, theme, onThemeToggle }) {
  ...
  const handleSubmit = async (formData) => {
    if (cooldown > 0) {
      setErrorMessage(`Please wait ${cooldown} seconds before requesting again.`)
      return
    }
  ...
  const footerContent = (
    <div style={{ textAlign: 'center' }}>
      {cooldown > 0 && (
        <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--text-muted)', marginBottom: 'var(--spacing-sm)' }}>
